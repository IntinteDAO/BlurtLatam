...
export default class HelpContent extends React.Component {
    ...
    render() {
        ...
        let value = HelpData[this.locale][path];
        if (!value && this.locale !== 'en') {
            console.warn(
                `missing path '${path}' for locale '${this.locale}' help files, rolling back to 'en'`
            );
            value = HelpData.en[path];
        }
        if (!value) {
            console.error(
                `help file not found '${path}' for locale '${this.locale}'`
            );
            return null;
        }
